...
namespace Project_Work_My_Telegram_bot
{
    ...
    public class MessageProcessing
    {
        ...
        public async Task OnTextMessageAsync(Message message)
        {
            ...
            _users[chatId] = await DataBaseHandler.GetUserAsync(chatId) ?? new ClassDB.User();
            switch (message.Text)
            {
            ...
            switch (text)
            {
                ...
                default:
                    ...
                    else
                    {
                        ...
                        replyMarkup: new ReplyKeyboardRemove());
                        OnCallbackQueryMessage -= ChoiceCarPathAsync;
                        //Доделать каст регистрации машины 
        ...
        private async Task ClosedEnterProfilAsync(Message msg)
        {
            ...
            switch (text)
            {
                case "ДА":
                    ...
                    carDrive.PersonalId = (carDrive.isPersonalCar) ? chatId : null;
                    ...
                    else //Тут логиа должна быть обновления Автомобиля 
                    {
                        await _botClient.SendMessage(
                         chatId: chatId,
                         text: $"Машина и пользователь с таким номером в базе сществует обновить информацию?",
